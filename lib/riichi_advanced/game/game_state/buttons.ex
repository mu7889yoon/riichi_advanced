defmodule RiichiAdvanced.GameState.Buttons do
  ...
  def to_buttons(state, button_choices) do
    ...
    unskippable_button_exists =
      Enum.any?(button_names, &Map.get(buttons[&1], "unskippable", false))

    if not Enum.empty?(button_names) and not unskippable_button_exists do
      button_names ++ ["skip"]
    else
      button_names
    end
  ...
  def make_button_choices(state, seat, button_name, button) do
    ...
    cond do
      Enum.any?(choice_actions, fn [action | _opts] ->
        action in ["call", "self_call", "upgrade_call", "flower", "draft_saki_card"]
      end) ->
        ...
        is_upgrade =
          Enum.any?(choice_actions, fn [action | _opts] -> action == "upgrade_call" end)

        is_flower = Enum.any?(choice_actions, fn [action | _opts] -> action == "flower" end)

        is_saki_card =
          Enum.any?(choice_actions, fn [action | _opts] -> action == "draft_saki_card" end)
        ...
        {state, call_choices} =
          cond do
            is_upgrade ->
              upgrade_names = List.wrap(button["upgrades"])

              call_choices =
                state.players[seat].calls
                |> Enum.filter(fn {name, _call} -> name in upgrade_names end)
                |> Enum.map(fn {_name, call} ->
                  Enum.map(call, &Utils.add_attr(&1, ["_hand", "_called"]))
                end)
                |> Enum.map(fn call_tiles ->
                  Riichi.make_calls(button["call"], call_tiles, tile_behavior, hand ++ draw)
                end)
                |> Enum.flat_map(
                  &Enum.map(&1, fn {called_tile, call_choice} ->
                    %{Utils.strip_attrs(called_tile) => call_choice}
                  end)
                )
                |> Enum.reduce(%{}, fn call_choices, acc ->
                  Map.merge(call_choices, acc, fn k, l, r ->
                    Enum.uniq_by(l ++ r, &MapSet.new(Utils.strip_attrs([k | &1])))
                  end)
                end)
                |> Enum.uniq_by(fn {k, ccs} ->
                  Enum.map(ccs, fn cc -> MapSet.new(Utils.strip_attrs([k | cc])) end)
                end)
                |> Map.new()
            ...
            is_flower ->
              flowers =
                Enum.flat_map(choice_actions, fn [action | opts] ->
                  if action == "flower" do
                    opts
                  else
                    []
              ...
              flowers_in_hand =
                Enum.filter(state.players[seat].hand ++ state.players[seat].draw, fn tile ->
                  Utils.has_matching_tile?([tile], flowers)
                end)
            ...
            is_saki_card ->
              ...
              state =
                if Enum.empty?(cards) do
                  show_error(
                    state,
                    "WARNING: not enough sakicards in the deck to play sakicards!"
                  )
                else
                  state
                end
            ...
            true ->
              callable_tiles =
                if is_call do
                  Enum.take(state.players[state.turn].pond, -1)
                else
                  []
                end

              call_choices =
                Riichi.make_calls(button["call"], hand ++ draw, tile_behavior, callable_tiles)
                |> Enum.map(fn {called_tile, call_choice} ->
                  %{Utils.strip_attrs(called_tile, :invisible) => call_choice}
                end)
                |> Enum.reduce(%{}, fn call_choices, acc ->
                  Map.merge(call_choices, acc, fn _k, l, r ->
                    Enum.uniq_by(l ++ r, &Utils.strip_attrs(&1))
                  end)
                end)
                |> Enum.uniq_by(fn {k, ccs} ->
                  Enum.map(ccs, fn cc -> MapSet.new(Utils.strip_attrs([k | cc])) end)
        ...
        # filter call_choices
        call_choices =
          if Map.has_key?(button, "call_conditions") do
            conditions = button["call_conditions"]
            # TODO better way to check call_source
            call_source =
              if is_call do
                :discards
              else
            ...
            for {called_tile, choices} <- call_choices do
              ...
              {called_tile,
               Enum.filter(choices, fn call_choice ->
                 Conditions.check_cnf_condition(state, conditions, %{
                   seat: seat,
                   call_source: call_source,
                   choice: %Choice{
                     name: button_name,
                     chosen_called_tile: called_tile,
                     chosen_call_choice: call_choice
                   }
                 })
               end)}
            end
      ...
      Enum.any?(choice_actions, fn [action | _opts] -> action == "mark" end) ->
        [_ | opts] =
          Enum.filter(choice_actions, fn [action | _opts] -> action == "mark" end) |> Enum.at(0)

        mark_spec =
          Enum.at(opts, 0, [])
          |> Enum.map(fn [target, needed, restrictions] -> {target, needed, restrictions} end)

        {state,
         {:mark, mark_spec, Enum.at(opts, 1, []), Enum.at(opts, 2, []), Enum.at(opts, 3, [])}}
  ...
  def recalculate_buttons(state, interrupt_level \\ 100) do
    ...
    if state.game_active and Rules.has_key?(state.rules_ref, "buttons") do
          ...
          {state, new_button_choices} ->
            if Actions.performing_intermediate_action?(state, seat) do
              ...
              button_choices =
                ...
                |> Enum.filter(fn {name, button} ->
                  ...
                  if Debug.debug_buttons() do
                    IO.puts(
                      "recalculate_buttons: at #{inspect(System.os_time(:millisecond) - t)} ms, checking #{name} for #{seat}"
                    )
                  end
                  ...
                  Map.get(button, "interrupt_level", 100) >= interrupt_level and
                    if name in Map.get(Debug.debug_specific_buttons(), seat, []) do
                      case Enum.find(
                             show_when,
                             &(not Conditions.check_cnf_condition(state, [&1], %{
                                 seat: seat,
                                 call_name: name,
                        ...
                        condition ->
                          IO.puts(
                            "Button #{name} for player #{seat}: failed condition #{inspect(condition)}"
                          )

                          false
                      end
                    else
                      Conditions.check_cnf_condition(state, show_when, %{
                        seat: seat,
                        call_name: name,
                        calls_spec: calls_spec,
                        upgrade_name: upgrades
              ...
              {state, button_choices} =
                for {name, button} <- button_choices, reduce: {state, []} do
                  {state, button_choices} ->
      ...
      new_button_choices =
        for {seat, button_choices} <- new_button_choices, into: %{} do
          button_choices =
            state.players[seat].button_choices
            |> Enum.filter(fn {name, _spec} ->
              (get_in(buttons[name]["interrupt_level"]) || 100) < interrupt_level
            end)
            |> Map.new()
      ...
      # IO.inspect(buttons)
      all_buttons =
        Map.new(new_button_choices, fn {seat, button_choices} ->
          {seat, to_buttons(state, button_choices)}
        end)
    ...
    if Enum.member?(state.players[seat].buttons, button_name) do
      ...
      actions =
        if button_name == "skip" do
          ...
          Rules.get(state.rules_ref, "buttons", %{})[button_name]["actions"]
        end

      state = Actions.submit_actions(state, seat, button_name, actions)
      # show possible call buttons
      state = broadcast_state_change(state)
      state
    else
      IO.puts(
        "#{seat} tried to press nonexistent button #{button_name}; available buttons are #{inspect(state.players[seat].buttons)}"
      )
  ...
  def trigger_auto_button(state, seat, auto_button_name, enabled) do
    ...
    if enabled do
      :timer.apply_after(100, GenServer, :cast, [
        self(),
        {:trigger_auto_button, seat, auto_button_name}
      ])
